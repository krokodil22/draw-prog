...
function updateSuccessText(){const ok=checkCompletion();const isFilled=drawnSegments.length===expectedPath.length;successMessage.textContent=ok?'Поздравляем! Рисунок выполнен правильно!':(isFilled?'Неверно. Попробуй еще раз':'');successMessage.classList.toggle('visible',Boolean(successMessage.textContent));setTaskCompleted('draw',ok);if(ok) checkAllTasksCompletion();}
...
function checkLogicTask(){const allFilled=Array.from(logicInputs).every((input)=>input.value.trim()!=='');const allCorrect=allFilled&&Array.from(logicInputs).every((input)=>input.value===logicAnswers[input.dataset.animal]);logicSuccessMessage.textContent=allFilled?(allCorrect?'Выполнено':'Неверно. Попробуй еще раз'):'';setTaskCompleted('logic',allCorrect);return allCorrect;}
