...
function updateSuccessText(){const ok=checkCompletion();const isFilled=drawnSegments.length>=expectedPath.length;const isWrong=isFilled&&!ok;successMessage.textContent=ok?'Поздравляем! Рисунок выполнен правильно!':(isWrong?'Неверно. Попробуй еще раз':'');successMessage.classList.toggle('visible',Boolean(successMessage.textContent));successMessage.classList.toggle('success-overlay--error',isWrong);setTaskCompleted('draw',ok);if(ok) checkAllTasksCompletion();}
...
function checkLogicTask(){const allFilled=Array.from(logicInputs).every((input)=>input.value.trim()!=='');const allCorrect=allFilled&&Array.from(logicInputs).every((input)=>input.value===logicAnswers[input.dataset.animal]);const isWrong=allFilled&&!allCorrect;logicSuccessMessage.textContent=isWrong?'Неверно. Попробуй еще раз':'';logicSuccessMessage.classList.toggle('logic-success--error',isWrong);setTaskCompleted('logic',allCorrect);return allCorrect;}
...
function checkMonsterTask(){const placed=Array.from(houseSlots).flatMap((slot)=>Array.from(slot.querySelectorAll('.monster')).map((monster)=>({house:slot.dataset.house,monster:monster.dataset.monster})));if(placed.length!==monsters.length){monstersSuccessMessage.textContent='';monstersSuccessMessage.classList.remove('logic-success--error');setTaskCompleted('monsters',false);return false;}const allCorrect=placed.every((p)=>monsterAnswers[p.monster]===p.house);houseSlots.forEach((slot)=>{const monster=slot.querySelector('.monster');if(!monster) return;const correctHouse=monsterAnswers[monster.dataset.monster];slot.classList.toggle('house-correct',correctHouse===slot.dataset.house);slot.classList.toggle('house-wrong',correctHouse!==slot.dataset.house);});const isWrong=!allCorrect;monstersSuccessMessage.textContent=isWrong?'Неверно. Попробуй еще раз':'';monstersSuccessMessage.classList.toggle('logic-success--error',isWrong);setTaskCompleted('monsters',allCorrect);return allCorrect;}
